...
#'        base R Markdown HTML output formatter
#' @export
FT <- function(number_sections = FALSE,
               fig_width = 7,
               fig_height = 5,
               fig_retina = if (!fig_caption) 2,
               fig_caption = FALSE,
  ...
  dep <- htmltools::htmlDependency(
    name = "FT",
    version = "0.1.0",
    system.file("rmarkdown", "templates", "FT", "resources", package = "hrbrthemes"),
    # script="rainbow-custom.min.js",
    stylesheet="FT.css"
  )
  ...
  args <- c(
    args, "--template",
    rmarkdown::pandoc_path_arg(
      system.file("rmarkdown", "templates", "FT", "base.html", package = "hrbrthemes")
    )
  )
